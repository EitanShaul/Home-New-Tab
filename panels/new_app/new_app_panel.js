...
var defaultRoundedCorner = 0.17*128;
...
function saveNewApp(name, cropbox) {
  function saveWithIcons(savedFilename) {
    var icons = savedFilename ? [{ size: 128, url: savedFilename }] : null;
    var borderRadiusPx = $('#imgWrapper').css('border-radius');
    if (icons && icons[0])
      icons[0].borderRadius = parseInt(borderRadiusPx, 10) || (void 0);
    bg_call('add_user_app', name, currentApp.url, icons);
  }
  (cropbox) ? saveAppIcon(cropbox, saveWithIcons) : saveWithIcons(null);
}

function saveEditedApp(name, cropbox) {
  function finishSaving(iconURL) {
    var icons = iconURL ? [{ size: 128, url: iconURL }] : null;
    var borderRadiusPx = $('#imgWrapper').css('border-radius');
    icons[0].borderRadius = parseInt(borderRadiusPx, 10) || (void 0);
    bg_call('edit_user_app', appBeforeEdits.id, name, currentApp.url, icons);
  }
  var newImage = $('#imageURL').val() || $('#imageFile').val();
  var beforeURL = appBeforeEdits.icons ? appBeforeEdits.icons[0].url : null;
  if (newImage != beforeURL)
    saveAppIcon(cropbox, finishSaving);
  else 
    finishSaving(beforeURL);
}

function saveAppIcon(cropbox, callback) {
  var dataURI = cropbox.getDataURL();
  let iconFileName = 'icon_' + randomToken() + '.png';
  save_file(iconFileName, dataURI, callback);
}
...
function searchForIcon() {
  ...
  var hasTLD = url.indexOf('.') != -1;

  // check invalid URL
  if (!hasTLD && url.indexOf('localhost') == -1 && !hasProtocol) 
    return;
...
function processSiteName(doc) {
  ...
  $('#name').val(siteName).focus();
}

function getSiteName(doc) {
  // { attr1: [{content: '...'}, {content: '...'}, ...], attr2: ... }
  // -> shortest content value
  return Object.values(getMetaTags(doc, titleAttributeValues))
    .flat()
    .map(tag => tag.content)
    .filter(Boolean)
...
$('#menu').click(function (e) {
  if ('menu-item-custom' == e.target.id) {
    switchToCustom();
  } else {
    switchToStore();
  }
...
function switchToStore() {
    $('#menu > .active')[0].classList.remove('active');
    $('#menu-item-store')[0].classList.add('active');
    $('#store').show();
    $('#custom').hide();
    $('#search-term').focus(); 
}
...
function switchToCustom() {
    $('#menu > .active')[0].classList.remove('active');
    $('#menu-item-custom')[0].classList.add('active');
    $('#store').hide();
    $('#custom').show();
    $('#url').focus(); 
}
...
function initCustomEditor(app) {
  currentApp.url = app.link;
  $('#name').val(app.name);
  $('#url').val(app.link);
  if (app.icon) {
    $('#imageURL').val(app.icon);
    downloadAndApplyImageWithURL(app.icon, function () {
      $('#roundedCorner').val(defaultRoundedCorner);
      $('#roundedCorner').trigger('input');
    });
  }
  $('input[type=range]').prop('disabled', false);
...
if (showPlease) {
  byId('all-thanks').style.display = 'block';
}
