  ...
  */
  ...
  function keydown(e) {
      ...
      if (is_first_keydown) {
        is_first_keydown = false;
        try {
          const conf = getSearchConfig();
          (new Image()).src = `https://${conf.search_domain}/favicon.ico`;
        } catch (e) { }
      }
      ...
      if (key == 17 || key == 16 || key == 91) { // Ctrl | Shift | CMD
        ...
        const isAISearch = e.ctrlKey || e.metaKey;
        if (isAISearch) {
          show_ai_chat_panel(q.value);
          suggestions.style.display = 'none';
        } else {  
          search(e);
        }
  ...
  function term_changed(e) {
    ...
    if (term && term.length < 100) { // 100 char limit for Google suggest
      ...
      var url = 'https://google.com/complete/search?client=firefox&q=' + // &hl=en
      ...
      function updateSuggestions() {
        ...
        // Add AI suggestion as last item
        var aiTerm = term;
        if (aiTerm) {
          var shortcutHtml = '<span class="shortcut-hint">' +
            '<span class="key cmd"></span> + ' +
            '<span class="key enter"></span>' +
            '</span>';
        
          html += '<li class="ai-suggest" data-term="' + aiTerm + '" data-ai="true">' +
                  '<span class="ai-icon">' + getIconAI() + '</span>' +
                  '<span class="ai-prefix">Ask AI:</span> ' + aiTerm +
                  shortcutHtml +
                  '</li>';
        }

        suggestions.innerHTML = html;
        suggestions.style.display = (words.length || topHit || aiTerm) ? "block" : "none";
      }
    } else {
      suggestions.style.display = "none";
  ...
  function search(e) {
    ...
    if (!term) return;
    // AI search
    var activeItem = $('.active', suggestions)[0] || e.target.closest('li');
    if (activeItem && activeItem.dataset.ai) {
      show_ai_chat_panel(term);
      suggestions.style.display = 'none';
      return;
    }
    // full url, naked url, or ip address
...
//  var feed = Feed.getInstance('gsp_hnt_00_00');
  searchOverride = function (term) {
    ...
    ga_cb('search.send', 'event', 'search', 'search-yahoo-direct', function () {
      ...
      //window.location = 'https://search.yahoo.com/search?isource=homenewtab&iname=yahoo&itype=web&p=' + encodeURIComponent(term);
      // this is the last "working" one (2021 dec 14)
      ///window.location = "https://searchdinam.com/?a=gsp_hnt_00_00&q=" + encodeURIComponent(term);
      //window.location = "https://searchmake.com/?a=gsp_hnt_00_00&q=" + encodeURIComponent(term); // yahoo, josh
      //window.location = "https://searc.me/11a66df4-480a-4425-b9a4-b5c8c1d7eb4e?chname=46470&q=" + encodeURIComponent(term); // google, korac
      try {
        var conf = getSearchConfig();
        window.location = conf.search_url.replace('${query}', encodeURIComponent(term)); 
      } catch (e) {
        window.location = "https://searchmake.com/?a=gsp_hnt_00_00&q=" + encodeURIComponent(term);
      }
    });
    return true;
  };
...
function getIconAI() {
  return `<svg width=16 viewBox="0 0 32 32" xmlns="http://www.w3.org/2000/svg">
  <path fill="currentColor" d="M30 1.988H2c-1.105 0-2 .895-2 2v18.047c0 1.105.895 2 2 2h3.986v4.979c0 .404.242.767.614.922.125.052.257.078.386.078.258 0 .51-.099.701-.287l5.782-5.692H30c1.105 0 2-.895 2-2V3.988c0-1.104-.895-2-2-2zm0 20.046H12.649l-4.664 4.591v-4.591H2V3.987h28v18.047zM7 14.031h18c.552 0 1-.448 1-1s-.448-1-1-1H7c-.552 0-1 .448-1 1s.448 1 1 1zm0-4h18c.552 0 1-.448 1-1s-.448-1-1-1H7c-.552 0-1 .448-1 1s.448 1 1 1zm0 8.001h12c.552 0 1-.448 1-1s-.448-1-1-1H7c-.552 0-1 .448-1 1s.448 1 1 1z"></path>
</svg>`;
}

function createPreconnect(url) {
...
function preconnectToAISearch() {
  createPreconnect('https://ai.homenewtab.com');
}

// Add styles for AI suggestion
(function addAIStyles() {
  var style = document.createElement('style');
  style.textContent = `
    #suggestions li.ai-suggest {
      border-top: 1px solid rgba(255,255,255,0.1);
      background-color: rgba(70,140,255,0.05);
    }
    #suggestions li.ai-suggest .ai-prefix {
      color: #4A8EFF;
      font-weight: bold;
    }
    #suggestions li.ai-suggest.active .ai-prefix,
    #suggestions li.ai-suggest.active .ai-icon {
      color: #ffd100;
      color: #fff;
      color: #fcf6c2;
    }
    #suggestions li.ai-suggest .ai-icon {
      margin-left: -1.5em;
      margin-right: .5em;
    ...
    /* Keyboard shortcut hint for AI suggest */
    #suggestions li.ai-suggest.active .shortcut-hint {
      display: none;
    }
    
    #suggestions li.ai-suggest .shortcut-hint {
      float: right;
      opacity: 0.75;
      font-size: 12px;
      font-weight: normal;
    ...
    /* Key symbols */
    #suggestions li.ai-suggest .shortcut-hint .key {
      display: inline-flex;
      align-items: center;
    ...
    /* Special key icons */
    #suggestions li.ai-suggest .shortcut-hint .key.cmd::before {
      content: "⌘";
      font-family: system-ui;
    }

    #suggestions li.ai-suggest .shortcut-hint .key.enter::before {
      content: "↵";
      font-family: system-ui;
    }

    /* Windows version */
    html:not(.mac) #suggestions li.ai-suggest .shortcut-hint .key.cmd::before {
      content: "Ctrl";
      font-size: 10px;
    }
  `;
