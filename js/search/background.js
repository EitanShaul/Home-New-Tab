...
(async () => {
  ...
  var searchProviders = {
    ...
    'google': { 
      name: 'Google', icon: '/icons/search/google.webp',
      url:  'https://www.google.com/search?ie=utf-8&q=${query}' },
    'yahoo':  { 
      name: 'Yahoo',  icon: '/icons/search/yahoo.webp',
      url:  'https://search.yahoo.com/search?isource=homenewtab&iname=yahoo&itype=web&p=${query}'  },
    'bing':   { 
      name: 'Bing',   icon: '/icons/search/bing2.png',
      url:  'https://www.bing.com/search?isource=homenewtab&iname=bing&itype=web&q=${query}'  },
    'duckduckgo':   { 
      name: 'Duck (DDG)', icon: '/icons/search/duckduckgo.webp',
      url:  'https://duckduckgo.com/?ia=homenewtab&q=${query}' },
    'baidu':  { 
      name: 'Baidu',  icon: '/icons/search/baidu.webp',
      url:  'https://www.baidu.com/baidu?isource=homenewtab' +
            '&iname=baidu&itype=web&tn=baidu&ch=7&ie=utf-8&wd=${query}'  },
    'yandex': { 
      name: 'Yandex', icon: '/icons/search/yandex.webp',
      url:  'https://yandex.com/search/?isource=homenewtab&itype=web&iname=yandex&text=${query}'  }
  };
  localStorage.SEARCH_providers = JSON.stringify(searchProviders);
})();


// ------------------------------------------------------------

// TODO: rest of code needs update for manifest v3
...
var SEARCH_LOG_SPEED = window.DEV;
			...
			window.db.transaction(function (tx) {
				...
				function calculationsFinished() {
					...
					window.db.transaction(function (tx) {
						...
						ga('search.send', 'event', 'search', 'index', 'history', toInsert.historyItems.length);
						...
						ga('search.send', 'event', 'timing-search-index', 'history', '', 
							 (+new Date-startTime));
...
function onHistoryVisited(historyItem) {
	...
	// history.onVisisted fires before page loaded, so to keep loading speedy
	// we delay our bookeeping
	setTimeout(function () {
...
function addVisit(url, title, visitId) {
	...
	if (SEARCH_LOG_SPEED) console.time('addVisitOld');
	...
	chrome.history.getVisits({ url: url }, function (visitItems) {
		...
		window.db.transaction(function (tx) {
			...
			errorHandlerDatabase(err, getLineInfo());
		}, function txSuccess() { if (SEARCH_LOG_SPEED) console.timeEnd('addVisitOld'); });
...
var newInstall = true; /// MAY !localStorage.install_time || (+localStorage.install_time > newInstallTime);// 1492716798000
...
chrome.tabs.onCreated.addListener(function (tab) {
	...
	if (isNotMac) {
		...
		//chrome.tabs.update(tab.id, { url: 'https://www.homenewtabsearch.com/newtab.html' });
...
}, { urls: [ 'https://www.homenewtabsearch.com/newtab.html' ] }, [ "blocking" ]);
*/
...
}, { urls: [ 'https://www.homenewtabsearch.com/backButton.html*' ] }, [ "blocking" ]);
*/
...
chrome.webRequest.onSendHeaders.addListener(function (e) { 	
	ga('search.send', 'event', 'search', 'bg-search-start');
	searchRequests[e.requestId] = setTimeout(onSearchRequestTimeout, 5*SECONDS);
}, searchRequestFilter);

chrome.webRequest.onCompleted.addListener(function (e) { 	
	ga('search.send', 'event', 'search', 'bg-search-complete');
}, searchRequestFilter);
...
function onSearchRequestTimeout() {
	ga('search.send', 'event', 'search', 'bg-timeout-5');
}

function onSearchRequestError(message) {
	ga('search.send', 'event', 'search', 'bg-error', message);
}

function onSearchRequestAbort(message) {
	ga('search.send', 'event', 'search', 'bg-abort', message);
}
...
var searchFilterNav = { url: [ 
	{ urlPrefix: 'https://www.homenewtabsearch.com/i' },
...
/* 21 oct
chrome.webNavigation.onBeforeNavigate.addListener(function (e) { 
	ga('tile.send', 'event', 'tile-click-temp', e.url);
}, { url: [{ urlPrefix: 'https://homenewtab.ampxdirect.com/' }]});
*/
...
chrome.runtime.onInstalled.addListener(function (details) {
	if ('install' == details.reason) {
		localStorage.FB_search_promo_after = +new Date + 4*24*60*60*1000;
...
//
// Search API from background (fallback)
//
...
window.DEBUG_bing_search = 
function DEBUG_bing_search(term, page, onSuccess, onError, stopOnError) {
	var url = '/error001.html';
		...
		try {
			...
			results = [].map.call(results, function (res) {
				...
				try {
					...
					var descEl = res.getElementsByTagName('p')[0] || 
											 res.querySelectorAll('span[title]')[0] ||
											 res.querySelectorAll('.tab-content')[0];
					if (descEl) {
            item.Description = descEl.textContent; // span[title] is tabbed wiki
          } else {
            var afterAttri = res.querySelector('.b_attribution').nextElementSibling;
            item.Description = afterAttri.textContent.slice(0, 300);
          }
				} catch (e) {
					item.hadError = true;
					errorCounter += 1;
...
function getJSONResultsForTerm(term, page, onSuccess, onError, stopOnError) {
	...
	var url = 'https://www.bing.com/search?q=' + q + '&first=' + first; //+ '&mkt=en-US';
	...
	ajax(url, function (xhr) {
		try {
			...
			ga('search.send', 'event', 'Search Error', '#parse ' + term, String(e.stack));
			post(	'https://search.homenewtab.com/debug/' + 
		...
		ga('search.send', 'event', 'Search Error', '#network ' + term, elapsedTillError);
...
function getResponseTextForTerm(term, page, onSuccess, onError, stopOnError) {
	var q = encodeURIComponent(term);
	var first = 1 + (page-1) * 10; // *14 is default on homepage
	var url = 'https://www.bing.com/search?q=' + q + '&first=' + first;
	ajax(url, function (xhr) {
		onSuccess(xhr.responseText);
	}, function (xhr) {
		onError && onError(xhr.status + ' ' + xhr.statusText);
		ga('search.send', 'event', 'Search Error', '#network ' + term, xhr.status + ' ' + xhr.statusText);
	});
}
