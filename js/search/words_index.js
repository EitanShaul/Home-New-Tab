...
function indexWordsFromUrls(onSucess, onError) {
	...
	///(function (tx) { // readTransaction
		...
		function (e) {
			...
			console.log('SUCCESS');
			ga('search.send', 'event', 'search', 'index', 'words', allWords.length);
			ga('search.send', 'event', 'search', 'index', 'word_url_pairs', wordUrlPairs.length);
			ga('search.send', 'event', 'timing-search-index', 'words', '', elapsed);
