...
function read_file(file, callback, onerror) {
  read_file_with_type(file, 'dataURL', callback, onerror);
}

function read_text_file(file, callback, onerror) {
  read_file_with_type(file, 'text', callback, onerror);
}

function read_file_with_type(file, type, callback, onerror) {
  var reader = new FileReader();
  reader.onload = function (event) {
    callback(event.target.result);
  };
  reader.onerror = function (event) {
    onerror(event, event.target.result);
  };
  if ('dataURL' == type)
    reader.readAsDataURL(file);
  else
...
function imageURLToBlob(source, callback, onerror) {
  var oReq = new XMLHttpRequest();
  oReq.open("GET", source, true);
  oReq.responseType = "blob";
  oReq.onload = function(oEvent) {
    callback && callback(oReq.response, oReq, oEvent);
  };
  oReq.onerror = function(oEvent) {
    onerror && onerror(oReq.response, oReq, oEvent);
  };
  oReq.send();
}
